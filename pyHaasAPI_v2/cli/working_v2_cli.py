#!/usr/bin/env python3
"""
Working pyHaasAPI v2 CLI - Using v2 API Components

This is a proper v2 CLI that uses the v2 API components and services.
"""
...
from pyHaasAPI_v2 import (
    AsyncHaasClient, AuthenticationManager, Settings,
    LabAPI, BotAPI, AnalysisService, LabService, BotService,
    LabAnalysisResult, BotCreationResult, MassBotCreationResult,
...
class WorkingV2CLI:
    """Working v2 CLI using v2 API components"""
    ...
    async def connect(self) -> bool:
        """Connect using v2 authentication"""
        try:
            ...
            # Create v2 client with config
            from pyHaasAPI_v2.config.api_config import APIConfig
            from pyHaasAPI_v2.config.logging_config import LoggingConfig
            from pyHaasAPI_v2.core.auth import AuthenticationManager
            
            # Create logging config
            logging_config = LoggingConfig()
            
            # Create API config with logging
            config = APIConfig(
                host='127.0.0.1',
                port=8090,
    ...
    async def analyze_lab(self, lab_id: str, min_winrate: float = 55.0, sort_by: str = "roe") -> Dict[str, Any]:
        """Analyze a single lab with zero drawdown requirement using v2 services"""
        try:
            logger.info(f"📊 Analyzing lab: {lab_id[:8]}")
            
            # Use v2 analysis service
            result = await self.analysis_service.analyze_lab(
                lab_id=lab_id,
    ...
    async def analyze_all_labs(self, min_winrate: float = 55.0, sort_by: str = "roe") -> Dict[str, Any]:
        """Analyze all labs with zero drawdown requirement using v2 services"""
        try:
            logger.info("🔍 Analyzing all labs with zero drawdown requirement using v2 services...")
            
            # Get all labs using v2 API
    ...
    async def create_bots_from_analysis(self, lab_results: Dict[str, Any], bots_per_lab: int = 2) -> List[Dict[str, Any]]:
        """Create top bots for each lab with zero drawdown using v2 services"""
        try:
            logger.info(f"🤖 Creating top {bots_per_lab} bots for each lab using v2 services...")
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Working pyHaasAPI v2 CLI - Zero Drawdown Analysis & Bot Creation (v2 API)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Analyze labs and create top 2 bots for each
  python pyHaasAPI_v2/cli/working_v2_cli.py --analyze --create-bots --bots-per-lab 2
  
  # Analyze with custom win rate threshold
  python pyHaasAPI_v2/cli/working_v2_cli.py --analyze --min-winrate 60
  
  # Just analyze without creating bots
  python pyHaasAPI_v2/cli/working_v2_cli.py --analyze
        """
    )
