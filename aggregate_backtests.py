...
def parse_report(file_path):
    try:
        ...
        # 1. Basic Metadata
        lab_id = content.get("LabID", content.get("AID", "Unknown"))
        market = content.get("PriceMarket", content.get("M", "Unknown"))
        
        # 2. Raw Trade Data
        finished_positions = content.get("FinishedPositions", [])
        
        # 3. Calculate Custom Metrics
        custom_metrics = calculate_metrics(finished_positions)
        
        if not custom_metrics:
            return None # Skip files with no trades
...
def main():
    ...
    with tqdm(total=len(files)) as pbar:
        for result in pool.imap_unordered(parse_report, files):
            if result:
    ...
    print(f"📊 Processed {len(results)} valid accounts with trades.")
