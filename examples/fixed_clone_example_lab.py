#!/usr/bin/env python3
"""
Fixed Clone Example Lab to Markets

This script clones the existing "Example" lab to the target markets and properly sets config parameters.
"""
...
def main():
    ...
    # Find the Example lab (use the non-running one)
    ...
    # Use the non-running Example lab
    example_lab = None
    for lab in example_labs:
        if lab.status.value == 0:  # CREATED status
            example_lab = lab
            break
    
    if not example_lab:
        example_lab = example_labs[0]  # Use first one if no non-running found
    ...
    correct_config = LabConfig(
        max_population=10,    # Max Population
        max_generations=100,  # Max Generation
        max_elites=3,         # Max Elites
        mix_rate=40.0,        # Mix Ratio
        adjust_rate=25.0      # Adjust Ratio
    )
    
    logger.info(f"📋 Correct config to apply: {correct_config}")
    ...
    for i, market_pair in enumerate(target_markets, 1):
        try:
            ...
            # Set the correct config parameters
            lab_details.config = correct_config
            ...
            logger.info(f"  📋 Config applied: {updated_lab.config}")
