...
import dataclasses
from typing import Any, Dict, List, Optional, Generic, Literal, TypeVar, Type
...
class LabSettings(BaseModel):
    ...
    chart_style: int = Field(alias="chartStyle")
    order_template: int = Field(alias="orderTemplate")
    script_parameters: dict = Field(alias="scriptParameters", default_factory=dict)
...
class LabUpdateConfig(BaseModel):
    """Configuration for lab updates"""
    max_population: int = Field(alias="MP", default=10)
    max_generations: int = Field(alias="MG", default=10)
    max_elites: int = Field(alias="ME", default=3)
    mix_rate: float = Field(alias="MR", default=0.0)
    adjust_rate: float = Field(alias="AR", default=0.0)

class LabUpdateSettings(BaseModel):
...
class LabUpdateRequest(BaseModel):
    """Request model for updating lab details"""
    lab_id: str = Field(alias="labId")
    config: LabUpdateConfig = Field(default_factory=LabUpdateConfig)
    settings: LabUpdateSettings = Field(default_factory=LabUpdateSettings)
    name: Optional[str] = None

# Add this class after LabBacktestSummary and before LabDetails
class LabBacktestResult(BaseModel):
    ...
    status: BacktestStatus = Field(alias="Status")  # Using our BacktestStatus enum
    ...
    summary: LabBacktestSummary = Field(alias="Summary")  # Using our LabBacktestSummary model
