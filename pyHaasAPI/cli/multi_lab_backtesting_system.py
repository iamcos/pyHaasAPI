#!/usr/bin/env python3
"""
Multi-Lab Backtesting System for pyHaasAPI v2

This system can analyze multiple labs, extract best parameters, and run comprehensive backtesting workflows.
"""
...
class MultiLabBacktestingSystem(BaseCLI):
    """Multi-lab backtesting system using v2 server manager and v1 API"""
    ...
    async def analyze_lab(self, lab_id: str, top_count: int = 100) -> Dict[str, Any]:
        """Analyze a single lab to find best backtest results"""
        try:
            print(f"🔍 Analyzing lab: {lab_id[:8]}")
            
            # Get lab details
            lab_details = api.get_lab_details(self.executor, lab_id)
            lab_name = getattr(lab_details, 'lab_name', f'Lab {lab_id[:8]}')
            script_id = getattr(lab_details, 'script_id', 'Unknown')
            ...
            analysis = {
                ...
                'all_qualifying_backtests': [
                    {
                        'backtest_id': getattr(bt, 'backtest_id', ''),
                        'roi_percentage': getattr(bt, 'roi_percentage', 0),
    ...
    async def analyze_multiple_labs(self, lab_ids: List[str]) -> Dict[str, Any]:
        ...
        try:
            print(f"🔍 Analyzing {len(lab_ids)} labs...")
            
            results = {}
            for lab_id in lab_ids:
                try:
                    analysis = await self.analyze_lab(lab_id)
                    results[lab_id] = analysis
                except Exception as e:
                    print(f"❌ Failed to analyze lab {lab_id}: {e}")
    ...
    async def create_optimized_lab(self, source_lab_id: str, target_lab_id: str, 
        ...
        try:
            ...
            target_name = getattr(target_lab, 'lab_name', f'Lab {target_lab_id[:8]}')
            
            # Create new lab name
            new_lab_name = f"{target_name} - {coin_symbol} - Optimized"
    ...
    async def run_comprehensive_backtesting_workflow(self, lab_pairs: List[Tuple[str, str, str]]) -> Dict[str, Any]:
        """Run comprehensive backtesting workflow for multiple lab pairs"""
        try:
...
async def main():
    """Main entry point"""
    system = MultiLabBacktestingSystem()
